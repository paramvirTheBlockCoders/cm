...
const MyRefferal = () => {
  ...
  const handleAddChips = async (e) => {
    try {
      ...
      // api call
      let res = await axios.post("/api/updateround",data );
      const response = res.data;
                ...
                <tbody>
                  {referrals.length > 0 ? (
                    referrals.map((referral, index) => (
                      <tr key={referral.id}>
                        <td>{index + 1}</td>
                        <td>{referral.start_time} Sec</td>
                        <td>{referral.entry_fee}</td>
                        <td>{referral.firstprize}</td>
                        <td> {referral.lobbytype === "1v1 Battle" ? 2 : 4} </td>
